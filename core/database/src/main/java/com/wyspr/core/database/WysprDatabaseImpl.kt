...
/**
 * Default [WysprDatabase] implementation.
 *
 ...
 *    SECURITY-MODEL.md §4.
 */
class WysprDatabaseImpl(
    private val appContext: Context,
    private val keystore: KeystoreManager,
) : WysprDatabase {

    @Volatile
    private var room: WysprRoomDatabase? = null
    ...
    override suspend fun rekey(newPassphrase: ByteArray) = openLock.withLock {
        withContext(Dispatchers.IO) {
            val oldKey = passphrase?.copyOf()
                ?: keystore.deriveSubkey(DB_KEY_INFO, length = 32)
            room?.close()
            room = null
            passphrase?.fill(0)
            passphrase = null
    ...
    private fun requireOpen(): WysprRoomDatabase =
        room ?: error("WysprDatabase not open. Call open() first.")

    private companion object {
        const val DB_FILENAME = "wyspr.db"
