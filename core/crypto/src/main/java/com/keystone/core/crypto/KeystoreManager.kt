 ...
 *
 * Keystone refuses to run on devices without at least a TEE-backed keystore.
 ...
 * AndroidKeyStore provider from API 33+, and Keystone supports API 26+.
...
class AndroidKeystoreManager(
    ...
    @Volatile
    private var cachedHandle: KeystoneIdentityHandle? = null
    ...
    override fun loadOrCreateIdentityKey(): KeystoneIdentityHandle {
        cachedHandle?.let { return it }
        check(backing != KeystoreManager.Backing.SOFTWARE_REJECTED) {
            "Keystone requires hardware-backed key storage. Refusing to run."
        }
        ensureWrappingKey()
        ...
        try {
            ...
            secretKey.fill(0)
            val handle = KeystoneIdentityHandle(publicKey = publicKey, alias = IDENTITY_ALIAS)
            cachedHandle = handle
            return handle
        } finally {
            seed.fill(0)
     ...
     */
    class KeystoneIdentityHandle internal constructor(
        publicKey: ByteArray,
        ...
        override fun equals(other: Any?): Boolean {
            ...
            if (other !is KeystoneIdentityHandle) return false
    ...
    private companion object {
        const val IDENTITY_ALIAS = "keystone.identity.v1"
        const val WRAPPING_ALIAS = "keystone.identity.wrap.v1"
        const val PROVIDER_ANDROID_KEYSTORE = "AndroidKeyStore"
        const val SEED_FILENAME = "keystone.identity.seed.v1"
        ...
        val HKDF_SALT = "KEYSTONE/v1/HKDF-SALT".encodeToByteArray()
    }
}
